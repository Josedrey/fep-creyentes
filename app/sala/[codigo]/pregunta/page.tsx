...
export default function PantallaPregunta() {
  ...
  const recargarRespuestas = useCallback(async (salaId: string, preguntaActual: number) => {
    const { data: usuariosData } = await supabase
      .from("usuarios").select("id").eq("sala_id", salaId);
    const ids = (usuariosData || []).map((u) => u.id);
    if (ids.length === 0) return;
    const { data } = await supabase
      .from("respuestas").select("usuario_id")
      .eq("pregunta_id", preguntaActual).in("usuario_id", ids);
      ...
      canalSala = supabase
        ...
        .on("postgres_changes",
          ...
          async (payload) => {
            ...
            setSala(nueva);
            if (nueva.estado === "midiendo") { router.push(`/sala/${codigo}/midiendo`); return; }
        ...
        .on("postgres_changes",
          ...
          async () => {
            const { data: salaActual } = await supabase
              .from("salas").select("pregunta_actual").eq("id", salaData.id).single();
            if (salaActual) await recargarRespuestas(salaData.id, salaActual.pregunta_actual);
          }
        ).subscribe();
    })();
  ...
  async function responder(opcionIdx: number) {
    ...
    const { error: errResp } = await supabase.from("respuestas").insert({
      usuario_id: yo.id, pregunta_id: pregunta.id, opcion: opcionIdx,
    });
    if (errResp) { setError("No pudimos guardar tu respuesta."); setEnviando(false); setOpcionSeleccionada(null); return; }
    ...
    await supabase.from("usuarios").update({ vector_arquetipos: nuevoVector }).eq("id", yo.id);
    setEnviando(false);
  }
  ...
  async function avanzar() {
    ...
    if (siguiente >= PREGUNTAS.length) {
      await supabase.from("salas").update({ estado: "midiendo" }).eq("id", sala.id);
    } else {
      await supabase.from("salas").update({ pregunta_actual: siguiente }).eq("id", sala.id);
    }
  }
  ...
  return (
    <>
      <FondoLiquido colors={["#7B2CBF", "#D4145A", "#00E0FF"]} intensity={0.4} />
      <main className="min-h-screen flex flex-col p-6 pt-10 pb-8">

        <div className="w-full max-w-md mx-auto flex flex-col items-center gap-4 mb-8 fade-up">
          <div className="flex gap-2">
            {PREGUNTAS.map((_, i) => (
              <div
                key={i}
                className={`h-1 w-10 rounded-full transition-all duration-500 ${
                  i <= sala.pregunta_actual ? "bg-white" : "bg-white/20"
                }`}
              />
            ))}
          </div>
        ...
        <div className="w-full max-w-md mx-auto flex-1 flex flex-col">
          <h1
            key={pregunta.id}
            className="font-display text-center leading-[0.95] mb-10 fade-up"
            style={{ fontSize: "clamp(1.85rem, 7vw, 2.75rem)" }}
          >
            {pregunta.texto}
          </h1>

          <div className="flex flex-col gap-3 mb-8 fade-up-delay-1">
            {pregunta.opciones.map((opcion, idx) => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  className={`w-full text-left px-5 py-4 rounded-2xl border transition active:scale-[0.98] font-body ${
                    esSeleccionada
        ...
        <div className="w-full max-w-md mx-auto flex flex-col items-center gap-3 fade-up-delay-2">
          <p className="eyebrow">
          ...
          <div className="flex gap-2 mb-2">
            {usuarios.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  className={`w-2 h-2 rounded-full transition ${respondio ? "bg-white" : "bg-white/20"}`}
                  title={u.nombre}
                />
              );
            })}
          ...
          {yo.es_anfitrion ? (
            <button onClick={avanzar} disabled={!todosRespondieron} className="btn-primary w-full">
              {!todosRespondieron
                ? "Esperando al resto..."
                : esUltima ? "Revelar al grupo" : "Siguiente pregunta"}
            </button>
          ) : yaRespondi ? (
            <p className="text-center text-white/60 text-sm font-body">
              {todosRespondieron ? "Esperando al anfitrión..." : "Esperando al resto del grupo..."}
            </p>
          ) : (
